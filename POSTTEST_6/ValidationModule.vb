Module ValidationModule
    ...
    Public Function ValidasiDataProduk(ep As ErrorProvider, txtID As TextBox, txtNama As TextBox, cbJenis As ComboBox, txtStok As TextBox, txtHarga As TextBox, txtGaransi As TextBox, txtSpek As TextBox) As Boolean
        Dim idValid As Boolean = ValidasiTextBox(ep, txtID, "ID tidak boleh kosong")
        Dim namaValid As Boolean = ValidasiTextBox(ep, txtNama, "Nama Produk tidak boleh kosong")
        Dim jenisValid As Boolean = ValidasiComboBox(ep, cbJenis, "Jenis harus dipilih")
        Dim stokValid As Boolean = ValidasiStok(ep, txtStok, "Stok harus lebih besar dari 0")
        Dim hargaValid As Boolean = ValidasiTextBox(ep, txtHarga, "Harga wajib diisi")
        Dim garansiValid As Boolean = ValidasiTextBox(ep, txtGaransi, "Garansi Produk wajib diisi")
        Dim spesifikasiValid As Boolean = ValidasiTextBox(ep, txtSpek, "Spesifikasi wajib diisi")

        Return idValid And namaValid And jenisValid And stokValid And hargaValid And garansiValid And spesifikasiValid
    End Function
End Module